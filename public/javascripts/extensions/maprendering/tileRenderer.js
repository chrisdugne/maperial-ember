...
/*
 * a check avec vivien
 * couleur a generer avec layerId -> pour recup le style qui correspond
 * besoin de zoom ? 
 * 
 * couleur a mettre dans params['stroke'] ou params['fill']
 */
TileRenderer.ApplyStyle = function ( ctx , line , attr, layerId , zoomLevel , drawIn ) {
   ...
   try {
      ...
      if ( curLayer.layer != drawIn)   return

      for (var _s = 0 ; _s < curLayer.s.length ; _s++ ) {
         var curStyle = curLayer.s[_s]
         if ( zoomLevel >= curStyle.zmax && zoomLevel <= curStyle.zmin ) {
...
/**
 *  data = json qui contient toutes les donn�es de la map.
 *  data["l"] = <layers> = toutes les donn�es lieees au Layers
 *  			contient une liste de <layerGroup>
 *  <layerGroup> contient une liste de <layer> (ll) et une liste de sources (li�e)
...
TileRenderer.RenderLayers = function ( ctx , data , zoomLevel , begin ) {
   var beginAt;
   var limitTime = false;
   ...
   for (i = beginAt ; i < data["l"].length ; ++i ) {
      ...
      for ( var l = 0 ; l < ll.length ; ++l ) {
         ...
         for ( var li = 0 ; li < lines.length ; ++li ) {
            var line = lines[li]
            TileRenderer.ApplyStyle ( ctx , line , attr , cl , zoomLevel,"back") // back = Background
         }
      }
      if (limitTime) {
